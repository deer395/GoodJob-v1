...
| P1 | 我无法判断 AI 推荐和邮件关联是否可信。 | 全程 | Phase 2 / Phase 3A / Phase 3B | 无 JD 主排序保持可解释的本地 baseline；AI 仅在用户主动单岗位分析时使用。邮件理解以脱敏证据、多事项提案和逐项人工确认实现，待真实使用验证。 |
...
## 2026-08-12｜Phase 3A 岗位匹配实验结论

**机会：** 用户需要在大量无完整 JD 的岗位中稳定排序，又希望在具体岗位上获得更深的判断依据。

**结论：** 无 JD 主排序保留本地规则 + n-gram baseline。LLM 主评分和有限语义纠偏均经过独立人工 Eval；后者虽有整体排序改善，但未满足全部 false-high 门槛，因此不进入主排序。

**保留能力：** structured LLM 用于用户主动的单岗位/完整 JD 深度分析。技术可用不等于自动进入主流程；后续任何 AI 主决策能力均需独立人工验证。

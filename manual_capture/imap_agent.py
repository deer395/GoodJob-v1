...
from manual_capture.email_processing import candidate_email, dedup_key, email_excerpt, email_evidence
...
def text_part(message):
    ...
    return ''
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--once',action='store_true'); parser.add_argument('--dry-run',action='store_true'); parser.add_argument('--refresh-existing',action='store_true'); args=parser.parse_args()
    ...
    try:
        ...
        count=created=deduplicated=refreshed=0
        for uid in (data[0].split() if status=='OK' else [])[:50]:
            ...
            subject=decoded(message.get('Subject')); body=text_part(message)
            if not candidate_email(subject, body): continue
            count+=1
            ...
            payload={'dedup_key':key,'key_type':key_type,'subject':subject[:300],'snippet':email_excerpt(body),'evidence':email_evidence(body),'received_at':received,'message_id':message.get('Message-ID'),'sender_domain':(message.get('From','').split('@')[-1].split('>')[0]),'refresh_existing':args.refresh_existing}
            ...
            if response.get('created'):
                created+=1
            elif response.get('refreshed'):
                refreshed+=1
            else:
                deduplicated+=1
        print(f'candidate emails: {count}; dry-run={args.dry_run}')
        print('sync-diagnostics: '+json.dumps({'candidate_count':count,'created_count':created,'deduplicated_count':deduplicated,'refreshed_count':refreshed},ensure_ascii=False))
